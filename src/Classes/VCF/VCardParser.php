    ...
    protected function parse()
    {
        ...
        foreach ($lines as $line) {
            ...
            if (strtoupper($line) == "BEGIN:VCARD") {
                ...
                if (isset($cardData->abLabels)) {
                    $tabLabel = $cardData->abLabels;
                }
                $tabLabel["nolabel"] = '';
                ...
                if (isset($cardData->phone)) {
                    $newitem = [];
                    foreach ($cardData->phone as $key => $nTab) {
                        foreach ($nTab as $t) {
                            $newitem[] = [$key,$tabLabel[$t[0]], $t[1]];
                        }
                    }
                    $cardData->phone = $newitem;
                }

                // Email
                if (isset($cardData->email)) {
                    $newitem = [];
                    foreach ($cardData->email as $key => $nTab) {
                        foreach ($nTab as $t) {
                            $newitem[] = [$key,$tabLabel[$t[0]], $t[1]];
                        }
                    }
                    $cardData->email = $newitem;
                }

                // Address
                if (isset($cardData->address)) {
                    $newitem = [];
                    foreach ($cardData->address as $key => $nTab) {
                        foreach ($nTab as $t) {
                            $newitem[] = [$key,$tabLabel[$t[0]], $t[1]];
                        }
                    }
                    $cardData->address = $newitem;
                }
                unset($cardData->abLabels);
                ...
                // Try to detect a group
                if (preg_match('/^\w+\./', $line, $label) === 1) {
                ...
                switch (strtoupper($element)) {
                    ...
                    case 'ADR':
                        ...
                        $cardData->address[$key][] = [$currentLabel,$this->parseAddress($value)];
                        break;
                    case 'TEL':
                        if (!isset($cardData->phone)) {
                            $cardData->phone = [];
                        ...
                        $cardData->phone[$key][] = [$currentLabel,$value];
                        break;
                    case 'EMAIL':
                        if (!isset($cardData->email)) {
                            $cardData->email = [];
                        ...
                        $key = !empty($types) ? implode(';', $types) : 'default';
                        $cardData->email[$key][] = [$currentLabel,$value];
                        break;
                    case 'REV':
                    ...
                    case 'X-ABLABEL':
                        ...
                        $key = !empty($types) ? implode(';', $types) : 'default';
                        $cardData->abLabels[$currentLabel] = $value;
                        break;
                    case 'NICKNAME':
                        $cardData->nickname = trim($value);
                        break;
                    case 'ROLE':
                        $cardData->role = trim($value);
                        break;
                    default:
                        echo "Non géré : ".($element)."\n";
                        break;
