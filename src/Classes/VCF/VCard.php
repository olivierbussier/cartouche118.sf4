...
*/
	class VCard implements Countable, Iterator
	{
		...
		public function __construct($Path = false, $RawData = false, array $Options = null)
		{
			...
			{
                throw new Exception('vCard: Path undefined');
			}
			...
			else
			{
				...
				foreach ($Lines as $Line)
				{
					...
					if ($ItemIndex != null) {
                        $this->Data[$Key]['item' . $ItemIndex] = $Value;
                    } else {
                        $this->Data[$Key][] = $Value;
                    }
				}
				// Reconstruction des composants a partir de la liste items
                if (isset($items)) {
                    foreach ($items as $k => $item) {
                        foreach (array_keys($item) as $typ) {
                            switch ($typ) {
                                case 'email':
                                case 'tel':
                                case 'adr':
                                    $this->Data[$typ]['item' . $k]['label'] = $item['x-ablabel'];
                            }
                        }
                    }
                }
